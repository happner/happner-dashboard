...
grid_app.controller('meshDashBoardController', ['$scope', '$modal', '$window', function($scope, $modal, $window) {
    ...
    $scope.attachStatistics = function(){
      ...
      $scope.meshAPIClient.api.event.system.on('stats/system', function(emitted, meta){

        $scope.dataBindStatistics(emitted, meta);

      }, function(e){

        if (!e)
           $scope.meshAPIClient.api.exchange.system.activateStatistics(1000);
